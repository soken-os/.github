...
**The work packet to BUILD the F1 fix was dispatched end-to-end and reached a mechanically-verified `COMPLETE` — the CEC return leg is proven under the G6 sandbox with the G8 and D2/G8-3 hardening in force.** This is the run the earlier NEEDS_INPUT convergence attempt could not reach; it is the first COMPLETE that exercises both the capability-test skip gate (G8) and first-class new-file evidence (D2).

**Two distinct proofs — do not conflate them (finding H1, Codex):**

- **(a) The lane itself — PASSED.** The return leg, G8's capability-skip gate, and D2/G8-3 evidence integrity all held live, on the evidence recorded below. This is what the run was for.
- **(b) F1's functional behavior — NOT yet proven live.** The controller running at `starting_ref=f251de3` did **not** contain the worker's diff; the change exists only in the unpublished worktree artifact. At `COMPLETE`, `notification_outbox` row 11 for `phase3-p3-notification-tick-v2` was still `PENDING`, and the service was then stopped as instructed. F1's delivery-after-restart behavior is supported by the worker's gate tests but was **not exercised by this run**. It still requires publication plus one live restart proof showing a pre-existing `COMPLETE`/`PENDING` row becomes `DELIVERED` without the work item being reprocessed.
...
**What this proves.** G8: the worker's acceptance gate skips the capability-blocked tests instead of failing them, so the lane no longer stalls at NEEDS_INPUT on a confinement artifact. D2/G8-3: the two files the worker created were bound into the verified evidence bundle by controller-side re-hashing, and the worker's diff was never committed or pushed — commit custody and evidence-integrity both held. **The single-lane return leg is accepted; F1 as a shipped behavior is not** (see (b) above).

**Open gates.** Multi-lane concurrency — several build items routed at once, each with independent lease/epoch custody and no cross-lane fate-sharing — is the remaining gate **before the 10-reviewer fleet**. It is not the sole remaining CEC gate: single-lane operational closure additionally requires (i) publication of the F1 artifact plus the live restart-delivery proof in (b), and (ii) the independent liveness acceptance drill (unload the scheduler job → phone alarm with no human action), tracked in product PR #269. "CEC build finished" means all of these, not concurrency alone.
