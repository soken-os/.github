...
unified diff with the same command the controller uses to regenerate it -- so
the claim it returns is verified by the real `verify_code_change_claim`, not a
stub. A lane asked for `needs_input` returns a genuine non-completion claim, so
the harness can prove one lane failing does not disturb the others.
"""
...
def main() -> int:
    ...
    # 1. A real tracked edit, inside this lane's own worktree (cwd).
    ...
    # 2. Passing test output (the verifier requires "passed" and rejects " failed").
