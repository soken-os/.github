"""Seed the approved P4 packet (deterministic notification delivery tick (F1)) into the registry."""
...
from .packet import p3_packet


def seed(work_item_id: str = "phase3-p3-notification-tick") -> str:
    phase2_migrate()
    ...
    with psycopg.connect(database_url(), autocommit=True) as conn:
        ...
        conn.execute(
            ...
            VALUES (%s,'CEC','P4: deterministic notification delivery tick (F1)','CIRCUIT_BUILD',60,600,
                    'READY','NONE','CONTROLLER','phase3-bootstrap-controller',%s,0,%s,
                    'DISPATCH_READY','phase3-p3-seed',%s,%s::jsonb,%s::jsonb,
                    'ROUTINE')""",
            (
                work_item_id,
                uuid4(),
                ...
                json.dumps({"action": "LAUNCH_P3_PACKET"}),
                json.dumps(packet),
            ),
        )
    return work_item_id
